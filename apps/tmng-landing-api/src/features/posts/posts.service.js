...
import { db, users, categories, tags, postCategories, postsTags as postTags, posts, } from "../../lib/db";
export const postsService = {
    // List posts with pagination and filters
    async listPosts(env, query, isAdmin = false) {
    ...
    async getPostById(id, isAdmin = false) {
    ...
    async getPostBySlug(env, slug, isAdmin = false) {
    ...
    async incrementPostViews(env, slug, location) {
        ...
        if (location && location.country) {
            console.log(`Post view from ${location.city || "Unknown"}, ${location.country} (timezone: ${location.timezone})`);
        }
    ...
    async createPost(data, authorId) {
        ...
        if (categoryIds && categoryIds.length > 0) {
            await db.insert(postCategories).values(categoryIds.map((categoryId) => ({
                postId: newPost.id,
                categoryId,
            })));
        }
        ...
        if (tagIds && tagIds.length > 0) {
            await db.insert(postTags).values(tagIds.map((tagId) => ({
                postId: newPost.id,
                tagId,
            })));
        }
