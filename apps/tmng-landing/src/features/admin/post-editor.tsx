...
interface PostEditorProps {
  initialData?: Post;
  isNew?: boolean;
  postId?: string; // Added prop
}

export default function PostEditor({ initialData, isNew = false, postId }: PostEditorProps) {
  ...
  const [isLoading, setIsLoading] = useState(!!postId); // valid if postId provided
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [previewHtml, setPreviewHtml] = useState("");
  const [activeTab, setActiveTab] = useState<"write" | "preview">("write");

  // Fetch Post Data if postId provided
  useEffect(() => {
    if (postId) {
        setIsLoading(true);
        apiClient.get(`${ENDPOINTS.ADMIN.POSTS}/${postId}`)
            .then(res => {
                if (res.data.data) {
                    const data = res.data.data;
                    // API returns categories as objects, but form expects array of IDs
                    const categories = Array.isArray(data.categories)
                        ? data.categories.map((c: any) => (typeof c === 'object' ? c.id : c))
                        : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isNew ? ENDPOINTS.ADMIN.POSTS : `${ENDPOINTS.ADMIN.POSTS}/${postId || initialData!.id}`;
      const method = isNew ? "post" : "put";

      const res = await apiClient[method](url, formDataToSend, {
        headers: {
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-6 rounded-2xl space-y-4">
            ...
            <div>
              ...
              >
                <SelectTrigger className="w-full bg-black/20 border border-white/10 text-purple-200 focus:outline-none focus:border-fuchsia-500/50">
